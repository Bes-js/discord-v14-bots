...
module.exports = {
  ...
  execute: async (client, message, args, beş_embed) => {
    ...
    if ((type == 0) || (type == 5)) {
      ...
      mesaj.edit({ content: `> **${newChannel.name} Adlı [VOICE] Kanalı Kuruluyor Ve Rol İzinleri Ayarlanıyor!**` })
      const newOverwrite = [];
      for (let index = 0; index < data.writes.length; index++) {
        const veri = data.writes[index];
        newOverwrite.push({
      ...
      mesaj.edit({ content: `> **${newChannel.name} Adlı [CATEGORY] Kanalı Kuruluyor Ve Rol İzinleri Ayarlanıyor!**` })
      const newOverwrite = [];
      for (let index = 0; index < data.writes.length; index++) {
        const veri = data.writes[index];
        newOverwrite.push({
      ...
      mesaj.edit({content:`> **${data.name} Adlı Kanalı Type Değerleri Uyuşmadığı İçin Kuramıyorum!** *Desteklenen Type'lar Voice,Text Ve Category*`})
    }
  }
}
